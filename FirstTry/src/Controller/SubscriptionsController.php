...
class SubscriptionsController extends AbstractController
{
    ...
    public function testEventSubscription( Request $req, Event $event): Response
    {
        // Link user and occurrences
        ...
        if ($form->isSubmitted() && $form->isValid()){
            ...
            // Subscription for multiple occurrences
            foreach($form->get('eventOccurrences')->getData() as $occurrence){
                $subscription->addEventOccurrence($occurrence)
                            ->setSubscriptionDate(new DateTimeImmutable());
                // $newSubscription = new EventSubscription();
                // $newSubscription->setUserSubscriptor($this->getUser())
                //                 ->addEventOccurrence($occurrence)
                //                 ->setNumberParticipants($form->get('numberParticipants')->getData());
